...
uint8_t USART1_aRxBuffer[USART1_RXBUFFERSIZE];
#endif
...
{
	if (HAL_UART_Transmit(&UART1_Handler, (uint8_t *)buffer, length, 1000) == HAL_OK) 
	{
		return length;
	}
	else
...
void USART1_Init(uint32_t bound)
{
	...
	HAL_UART_Receive_IT(&UART1_Handler, (uint8_t *)USART1_aRxBuffer, USART1_RXBUFFERSIZE);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if (huart->Instance == USART1)
	{
		...
		if ((USART1_RX_STA & 0x8000) == 0)
		{
			...
			{
				/*Receive error(\n), start over*/
				if (USART1_aRxBuffer[0] != 0x0a)USART1_RX_STA = 0;
				/*Receive complete*/
	...
	if (huart->Instance == USART3)
	{
		...
		if ((USART3_RX_STA & 0x8000) == 0)
		{
			...
			{
				/*Receive error(\n), start over*/
				if (USART3_aRxBuffer[0] != 0x0a)USART3_RX_STA = 0;
				/*Receive complete*/
